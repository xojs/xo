...
async function createConfig({
   ...
   */
	for (const config of [...arrify(userConfigs), {...globalOptions}].filter(Boolean)) {
     ...
     */
      ...
      && Object.keys(config)[0] === 'ignores'
		...
		baseConfig.push(pick(config, ['files', 'ignores', 'languageOptions', 'plugins', 'settings', 'rules']));
	}
	...
	baseConfig.push({
		...
		languageOptions: {
			...
			parserOptions: {
				...
				project: path.resolve(cwd, tsconfig ?? globalOptions?.parserOptions?.project ?? 'tsconfig.json'),
			},
		},
		settings: {
			'import/resolver': {
