...
const mergeWithFileConfig = options => {
	...
	if (options.filename && isTypescript(options.filename)) {
		...
		const tsConfigExplorer = cosmiconfigSync([], {searchPlaces: ['tsconfig.json'], loaders: {'.json': () => ({})}});
		const {filepath: tsConfigPath} = tsConfigExplorer.search(options.filename) || {};
...
const mergeWithFileConfigs = async (files, options) => {
	...
	return [...(await pReduce(files, async (configs, file) => {
		...
		if (isTypescript(filepath)) {
			// Override cosmiconfig `loaders` as we look only for the path of tsconfig.json, but not its content
			const tsConfigExplorer = cosmiconfig([], {searchPlaces: ['tsconfig.json'], loaders: {'.json': () => ({})}});
			({filepath: tsConfigPath} = await tsConfigExplorer.search(filepath) || {});
			fileOptions.tsConfigPath = tsConfigPath;
			fileOptions.ts = true;
		}

		const cacheKey = JSON.stringify({xoConfigPath, enginesConfigPath, prettierConfigPath, hash, tsConfigPath});
...
const buildTSConfig = options => config => {
	if (options.ts) {
		...
		config.baseConfig.parser = '@typescript-eslint/parser';
