...
async function recursiveBuildTsConfig(tsConfig, tsConfigPath) {
	tsConfig = tsConfigAbsPaths(tsConfig, tsConfigPath);
...
const tsConfigAbsPaths = (tsConfig, tsConfigPath) => {
	if (Array.isArray(tsConfig.files)) {
		tsConfig.files = tsConfig.files.map(
			f => path.isAbsolute(f) ? f : path.resolve(path.dirname(tsConfigPath), f),
		);
	}
