...
const buildXOConfig = options => config => {
	...
	if (options.nodeVersion) {
		...
		config.baseConfig.rules['n/no-unsupported-features/es-builtins'] = options.rules?.['n/no-unsupported-features/es-builtins'] ?? ['error', {version: options.nodeVersion}];
		config.baseConfig.rules['n/no-unsupported-features/es-syntax'] = options.rules?.['n/no-unsupported-features/es-syntax'] ?? ['error', {version: options.nodeVersion, ignores: ['modules']}];
		config.baseConfig.rules['n/no-unsupported-features/node-builtins'] = options.rules?.['n/no-unsupported-features/node-builtins'] ?? ['error', {version: options.nodeVersion}];
	}
