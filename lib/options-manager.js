...
const buildConfig = opts => {
	...
	const nodeVersion = opts.engines && opts.engines.node && semver.validRange(opts.engines.node);
	...
	if (nodeVersion) {
		for (const rule of Object.keys(ENGINE_RULES)) {
			...
			for (const minVersion of Object.keys(ENGINE_RULES[rule]).sort(semver.compare)) {
				if (!semver.intersects(nodeVersion, `<${minVersion}`)) {
					config.rules[rule] = ENGINE_RULES[rule][minVersion];
				}
			}
		}
		config.rules['node/no-unsupported-features/es-builtins'] = ['error', {version: nodeVersion}];
		config.rules['node/no-unsupported-features/es-syntax'] = ['error', {version: nodeVersion, ignores: ['modules']}];
		config.rules['node/no-unsupported-features/node-builtins'] = ['error', {version: nodeVersion}];
	}
