...
const buildConfig = opts => {
	...
	// If the user set the `prettier` options then add the `prettier` plugin and config
	if (opts.prettier) {
		// The prettier plugin uses Prettier to format the code with --fix
		config.plugins = config.plugins.concat('prettier');
		// The prettier config override Eslint stylistic rules that are handled by Prettier
		config.baseConfig.extends = config.baseConfig.extends.concat('prettier');
		// The `prettier/prettier` rule report errors if the code is not formated in accordance to Prettier
		config.rules['prettier/prettier'] = 'error';
		// If the user has the React, Flowtype or Standard plugin, add the corresponding prettier rules overrides
		// See https://github.com/prettier/eslint-config-prettier for the list of plugins overrrides
		['react', 'flowtype', 'standard'].forEach(override => {
